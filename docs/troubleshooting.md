...
## После продления срок на 1 день меньше (26.04 → 25.05)

Причина: naive `end_date` из БД (UTC) через `.timestamp()` считался **локальным** временем сервера → на панели и после sync дата съезжала на день назад (TZ≠UTC).

Исправлено в коде (`utils/utc.py`, продление/sync). Обновите бота (stable/edge). Уже сдвинутые подписки автоматически не компенсируются — при жалобе админ может **+1 день** в карточке клиента.
