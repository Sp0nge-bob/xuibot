...
| | `crypt5_api` | Запрос на `crypto.happ.su` → `happ://crypt5/…` (рабочий формат в Happ) |
| | `crypt4_local` | **Устарел** — Happ отклоняет `happ://crypt4/`; бот автоматически переключает на `crypt5_api` |
...
**Рекомендация:** если нужно скрыть URL в Happ — **`crypt5_api`**. Формат `happ://crypt4/` в актуальных версиях Happ даёт ошибку «ссылка не валидна»; официальный API `crypto.happ.su` выдаёт только crypt5. Ссылки кэшируются в памяти бота после первого запроса.
