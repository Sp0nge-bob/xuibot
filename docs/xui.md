...
## Мультинода: что делает бот, что — панель

| Действие | Где |
|----------|-----|
| Создание клиента, expiry, трафик, `limitIp`, enable | **Только основная нода** (бот) |
| Разнос параметров на вторичные | **Панель 3x-ui** сама, с основной |
| Удаление клиента | **На каждой ноде отдельно** — ограничение панели; бот вызывает `remove_client_everywhere` |
| «Призраки» без attach к inbound | `clients/delOrphans` на **всех** включённых нодах при full sync |
| Лишние tg на основной (нет в БД) | По-прежнему точечно при full sync |

Автосинк (раз в сутки + при старте): выравнивание БД ↔ основная + очистка призраков на вторичных. Отключить: `/admin` → **Ноды** → «Выключить автосинк». Кнопка **«Синхронизировать вторичные»** работает всегда.
