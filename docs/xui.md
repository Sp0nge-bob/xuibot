...
Синхронизация клиентов на вторичные ноды — автоматически (очередь + полный sync раз в сутки). Автосинк можно отключить в `/admin` → **Ноды** → «Выключить автосинк»; кнопка **«Синхронизировать вторичные»** при этом работает всегда.
