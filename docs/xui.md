[← Документация](README.md) · [Архитектура](architecture.md) · [Установка](installation.md) · [Конфигурация](configuration.md) · [Деплой](deployment.md) · [Platega](platega.md) · [Админка](admin.md) · [Подписки](subscriptions.md) · [Разработка](development.md) · [Troubleshooting](troubleshooting.md)

---

# 3x-ui и подписка

Бот работает на **Python 3.11–3.14**. Параметры клиента после create/update на Primary разносит **сама панель** на вторичные ноды; бот на extend не гоняет лишний `groups/bulkAdd`, если клиент уже в группе.

1. Бот подключается **только к главной панели** (`XUI_HOST`).
2. Вторичные ноды добавляются в админке → «Ноды».
3. На нодах подписка **выключена** — одна ссылка с главной панели.
4. `DEFAULT_SUBSCRIPTION_INBOUNDS` — по одному inbound с каждой ноды в общей подписке.
5. `SUBSCRIPTION_BASE_URL` — маскировка nginx (например `/api/v4/GET/`).

## Ссылка для клиента (Happ)

После оплаты бот отправляет ссылку подписки и QR. Функция `build_sub_link()` собирает URL из `SUBSCRIPTION_BASE_URL` + `{sub_id}`.

Опционально — шифрование для приложения Happ (см. [конфигурацию](configuration.md#happ--шифрование-ссылки-подписки)):

| Режим | Результат |
|-------|-----------|
| `none` | `https://sub.example.com/api/v4/GET/abc123` |
| `crypt3_local` | `happ://crypt3/…` — RSA-ключ из документации Happ |
| `crypt5_api` | `happ://crypt5/…` — через API crypto.happ.su |

Переключение: `/admin` → **Happ**. При ошибке шифрования бот отдаёт plain URL (fallback).

## Мультинода: что делает бот, что — панель

| Действие | Где |
|----------|-----|
| Создание клиента, expiry, трафик, `limitIp`, enable | **Только основная нода** (бот) |
| Разнос параметров на вторичные | **Панель 3x-ui** сама, с основной |
| Группы клиентов (`telegram-bot` / `web_client_group`) | **Основная нода**; при продлении веб-клиента в боте группа сохраняется |
| Лимит IP (`paid_limit_ip`) | Назначается **только при создании** новых клиентов; при продлении не перезаписывается |
| Удаление клиента | **На каждой ноде отдельно** — ограничение панели; бот вызывает `remove_client_everywhere` |
| «Призраки» без attach к inbound | `clients/delOrphans` на **всех** включённых нодах при full sync |
| Лишние tg на основной (нет в БД) | По-прежнему точечно при full sync |

Автосинк (раз в сутки + при старте): выравнивание БД ↔ основная + очистка призраков на вторичных. Отключить: `/admin` → **VPN** → **Ноды** → «Выключить автосинк». Кнопка **«Синхронизировать вторичные»** работает всегда.

Если вторичная нода offline — бот показывает пользователям короткое предупреждение (см. [Подписки](subscriptions.md)). Проверка: `/admin` → **Диагностика** → VPN.

При очень больших объёмах клиентов на панели 3x-ui может потребоваться **PostgreSQL на стороне панели** (отдельно от БД бота) — см. документацию 3x-ui.

## Утилиты

```bash
python scripts/list_inbounds.py    # список ID инбаундов
python scripts/dedupe_nodes.py     # убрать дубликаты нод в БД
```

| Путь | Назначение |
|------|------------|
| `scripts/list_inbounds.py` | ID инбаундов с панели |
| `scripts/dedupe_nodes.py` | Очистка дубликатов в БД |
| `scripts/dev/*` | Скрипты разработки — не использовать в проде |

---

**Назад:** [← Деплой](deployment.md) · **Далее:** [Platega →](platega.md)