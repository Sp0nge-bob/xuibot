...
async def is_sync_disabled() -> bool:
    """
    Отладка: отключить фоновую/плановую синхронизацию нод
    (старт, таймер, очередь после оплаты). Ручная кнопка «Синхронизировать
    вторичные» в админке работает всегда.
